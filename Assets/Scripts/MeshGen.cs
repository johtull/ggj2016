...
public class MeshGen : MonoBehaviour {
	...
	public void GenerateMesh(int[,] map, float squareSize){
        ...
        for(int i = 0; i < vertices.Count; i++){
            float percentX = Mathf.InverseLerp(-map.GetLength(0)/2*squareSize,map.GetLength(0)/2*squareSize,vertices[i].x) * tilingAmount;
            float percentY = Mathf.InverseLerp(-map.GetLength(0)/2*squareSize,map.GetLength(0)/2*squareSize,vertices[i].z) * tilingAmount;
	...
	void TriangulateSquare(Square square){
		...
		case 12:
			...
			break;
		case 5:
			MeshFromPoints(square.centerTop, square.topRight, square.centerRight, square.centerBottom, square.bottomLeft);
			break;
		case 10:
	...
	void CalculateMeshOutlines(){
		...
		for(int vertexIndex = 0;  vertexIndex < vertices.Count; vertexIndex++){
			if(!checkedVerts.Contains(vertexIndex)){
				int newOutlineVertex = GetConnnectedOuterVertex(vertexIndex);
	...
	void FollowOutline(int vertexIndex, int outlineIndex){
		...
		checkedVerts.Add(vertexIndex);
		int nextVertexIndex = GetConnnectedOuterVertex(vertexIndex);
	...
	int GetConnnectedOuterVertex(int vertexIndex){
		...
		for(int i = 0; i < trianglesContainingVertex.Count; i++){
			...
			for(int j = 0; j < 3; j++){
				...
				if(vertexB != vertexIndex && !checkedVerts.Contains(vertexB)){
					if(IsOuterEdge(vertexIndex, vertexB)){
					return vertexB;
					}
				}
			}
